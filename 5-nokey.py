...
st.title("樂活五線譜") # 🎯 需求 1: 大標題只留「樂活五線譜」
...
def render_metric_cards(current, fiveline_zone, action_detail):
    ...
    with st.container(border=True):
        ...
        col2.metric("五線譜位階", fiveline_zone)
...
def generate_internal_analysis(stock_name, stock_symbol, slope_dir, sd_level, fiveline_zone, current, sell_signals, buy_signals, full_bbw_series):
    ...
    if slope_dir == "上升": trend_summary = f"五線譜趨勢：明確為**上升**，股價位於 {fiveline_zone}。"
    elif slope_dir == "下降": trend_summary = f"五線譜趨勢：明確為**下降**，股價位於 {fiveline_zone}。"
    else: trend_summary = f"五線譜趨勢：**盤整或觀望**。"
    analysis_text.append(trend_summary + f" ADX({current_adx:.1f}) 顯示趨勢強度{'高' if current_adx > 30 else '中等' if current_adx > 20 else '弱'}。" + "\n")
    ...
    if current_williams_r > -20 and sell_signals: rec = f"**極度危險**：情緒超買且有 {len(sell_signals)} 個賣出訊號。建議投資人**立即清倉或空手**。"
    elif current_williams_r < -80 and buy_signals and current_adx < 25: rec = "**中線布局機會**：情緒極度悲觀。可考慮**極小額試單**，設嚴格止損。"
    elif current_bbw < bbw_quantile and current_adx < 20: rec = "**靜待時機**：市場處於暴風雨前的寧靜。建議保持場外觀望。"
    elif sell_signals: rec = f"鑑於當前有 {len(sell_signals)} 個賣出訊號，建議投資人**減碼或空手觀望**。"
    elif buy_signals: rec = f"當前有 {len(buy_signals)} 個買入訊號，建議可考慮**分批進場**，並緊盯 ADX 確認趨勢強度。"
    else: rec = "多數指標訊號不明確。建議**保持觀望**。"
    analysis_text.append(rec + "\n")
    
    analysis_text.append("### 4. 聲明與風險提示")
    analysis_text.append(f"本分析為基於多重技術指標的**程式碼硬編碼判斷**，不構成任何投資建議。所有交易決策請自行承擔風險。")
    return "\n".join(analysis_text)
