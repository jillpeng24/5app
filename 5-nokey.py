...
h3, h4 {
    ...
    margin-top: 0.2rem !important;    /* 比之前更靠上 */
    margin-bottom: 0.10rem !important; /* 減少與內文間距 */
}

/* 確保分析報告中的標題層次拉平，移除粗體並收斂間距 */
...
/* Metric 主字：粉紫強調，並縮小字體以完整顯示（對應第三張圖片需求） */
.css-1r6rthg {
    color: #A07C8C !important;
    font-weight: 600 !important;
    font-size: 1.4rem !important; /* 調小字級，避免文字被截斷 */
}
...
# ================= Plotly config: remove autoscale/resetScale buttons =================
PLOTLY_CONFIG = {
...
# (預設內嵌 FinMind Token；如需改為 st.secrets 或 env，告訴我)
...
def get_tw_stock_data_finmind(symbol, start_date, end_date, api_token=None):
    """
    使用 FinMind API 獲取台股歷史數據
    回傳 index 為日期、欄位 Open/High/Low/Close/Volume（首字大寫）
    """
    try:
        ...
        if not all(c in df.columns for c in cols_needed):
            ...
            df = df[['date'] + existing] if 'date' in df.columns else df
...
def render_fiveline_plot(valid_data, slope_dir, slope):
    ...
    st.caption("圖表工具說明：可放大/縮小、平移、截圖等。已移除自動縮放 (autoScale) 按鈕以避免誤觸。")
...
def render_lohas_plot(valid_data, current_price, current_ma20w):
    ...
    fig2.update_layout(title=f"樂活通道走勢圖 - {zone_text}", height=500, hovermode='x unified', template='plotly_white', showlegend=False)
...
def render_oscillator_plots(valid_data):
    ...
    fig3.update_layout(title="RSI 相對強弱指標", height=300, hovermode='x unified', template='plotly_white', showlegend=False)
    ...
    fig4.update_layout(title="KD 隨機指標", height=300, hovermode='x unified', template='plotly_white', showlegend=False)
...
def render_volatility_plots(valid_data, current):
    ...
    fig_adx.update_layout(title="趨向指標 ADX, +DI, -DI", height=300, hovermode='x unified', template='plotly_white', showlegend=False)
    ...
    fig_bbw.update_layout(title="布林帶寬度 (BBW)", height=300, hovermode='x unified', template='plotly_white', yaxis_title="BBW (%)", showlegend=False)
    ...
    fig_williams.update_layout(title="威廉指標 (Williams %R)", height=300, hovermode='x unified', template='plotly_white', showlegend=False)
    st.plotly_chart(fig_williams, use_container_width=True, config=PLOTLY_CONFIG)
