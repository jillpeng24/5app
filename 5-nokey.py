...
def calculate_bbw(close, period=20, std_dev=2):
    ...
    # BBW = (Upper Band - Lower Band) / Middle Band (MA)
...
def generate_internal_analysis(stock_name, stock_symbol, slope_dir, sd_level, fiveline_zone, current, sell_signals, buy_signals):
    ...
    current_v_ratio = current['Volume_Ratio'] # 成交量均線比

    # 1. 趨勢判斷 (Trend Analysis) - 加入 ADX
    ...
    # 2. 轉折點與波動性 (Turning Points & Volatility) - 加入 BBW & DMI
    analysis_text.append("### 2. 轉折點與波動性分析")
    
    volatility_analysis = ""
    # BBW 判斷極端收縮 (潛在爆發點)
    if current_bbw < current['BBW'].quantile(0.1): # 如果 BBW 處於歷史低 10%
        volatility_analysis += f"⚠️ **波動性極端收縮 (BBW {current_bbw*100:.2f}%)**，預期未來價格將有**大幅變動**。轉折點可能迫近。\n"

    # DMI 判斷買賣動能轉換
    if current_plus_di > current_minus_di and current_plus_di > 30 and current_adx > 25:
        volatility_analysis += f"✅ **DMI 多頭佔優 (+DI > -DI)**，且趨勢強勁，動能正在推升價格。\n"
    elif current_minus_di > current_plus_di and current_minus_di > 30 and current_adx > 25:
        volatility_analysis += f"🛑 **DMI 空頭佔優 (-DI > +DI)**，且趨勢強勁，價格壓力巨大。\n"
    else:
        volatility_analysis += f"⚪ DMI 顯示多空力量均衡，或趨勢強度不足以做出明確判斷。\n"
    
    analysis_text.append(volatility_analysis)
    
    # 3. 操作建議 (Trading Recommendation) - 整合所有訊號
    analysis_text.append("### 3. 綜合操作建議")
    
    if sell_signals and current_adx > 25:
        rec = f"鑑於當前有 {len(sell_signals)} 個賣出訊號，且 **ADX 趨勢強度高**，建議投資人**立即減碼或空手**。高強度趨勢下的賣出訊勢更具說服力。"
    elif buy_signals and current_adx > 20 and sd_level < 1.0:
        rec = f"當前有 {len(buy_signals)} 個買入訊號，且 ADX 確認趨勢正在發展。建議可考慮**分批進場**，並嚴守止損點。"
    elif current_bbw < current['BBW'].quantile(0.1) and current_adx < 20:
        rec = "⚠️ **極度盤整**：波動性收縮但 ADX 弱，建議**保持場外觀望**，避免在轉折點爆發前被洗出場。"
    else:
        rec = "多數指標訊號不明確，或訊號強度不足。建議**保持觀望**，等待價格在樂活通道或五線譜邊緣出現明確的壓力或支撐訊號。"
        
    analysis_text.append(rec + "\n")
    
    # 4. 風險提示
    analysis_text.append("### 4. 聲明與風險提示")
    analysis_text.append(f"本分析為基於多重技術指標 (KD/RSI/MACD/DMI/BBW) 的**程式碼硬編碼判斷**，**不依賴外部 AI**，且不構成任何投資建議。所有交易皆有風險，請獨立判斷。")
    analysis_text.append(f"關鍵數據：RSI: {current['RSI']:.1f}，K/D: {current['K']:.1f}/{current['D']:.1f}，ADX: {current['ADX']:.1f}。")
...
if analyze_button:
    if not stock_input:
        ...
        try:
            ...
            # 🌟 新增指標計算區
            with st.spinner("🔧 計算技術指標 (RSI, MACD, KD, ADX, BBW)..."):
                # 舊指標
            ...
            with st.spinner("🎯 生成買賣訊號..."):
                # 確保新指標也被納入檢查範圍
                valid_data = regression_data.dropna(subset=['MA20W', 'UB', 'LB', 'RSI', 'K', 'D', 'ADX', 'BBW'])
            ...
            # 🌟 顯示新的 ADX 指標
            col_adx, col_di = st.columns(2)
            col_adx.metric("ADX (強度)", f"{current['ADX']:.1f}", delta_color="off")
            col_di.metric("+DI/-DI", f"+{current['+DI']:.1f}/-{current['-DI']:.1f}", delta_color="off")
            ...
            # ==================== 圖表分頁 (保持不變) ====================
            tab1, tab2, tab3, tab4 = st.tabs(["🎼 五線譜", "🌈 樂活通道", "📊 技術指標", "🚀 波動與動能"]) # 新增一個 Tab
            ...
            with tab4:
                ...
                # 標註 BBW 歷史低點 (10% Quantile) 作為極端收縮的參考
