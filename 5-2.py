...
# 💡 優化 1: 使用 @st.cache_data 緩存股票下載結果
@st.cache_data(ttl=3600) # 設定緩存時間為 1 小時 (3600秒)
def load_stock_data(symbol, days):
    ...
    if stock_data.empty:
        return None, None, symbol # 返回 None 表示失敗
    ...
    return stock_data, stock_name, symbol
    ...
    if not stock_symbol:
        ...
        try:
            with st.spinner("📥 下載股票資料中..."):
                ...
                # 呼叫緩存的資料下載函數
                stock_data, stock_name, stock_symbol_actual = load_stock_data(stock_symbol, days)
                
                if stock_data is None:
                    st.error(f"❌ 無法取得 {stock_symbol} 的資料")
                    st.stop()
